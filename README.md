...
```text
=====================================================
  Hysteria2-LuoPo 管理面板 v26.8.3 |  快捷启动: hy2
=====================================================
...
- `6`：卸载清理（高风险操作）
- `7`：排障常用命令速查
- `8`：完整 Sing-box 模板输出（兼容 sing-box 1.12+ 新规则格式）
- `9`：环境健康检查 + 报告导出
- `10`：查看最近一次诊断报告
- `11`：手动备份/恢复配置
...
适合：没有域名，想快速用 IP 连通。  
要求：原生 Hysteria2 客户端必须开启 `insecure=true`，并使用脚本导出的 `pinSHA256` 固定证书。

注意：自签节点使用 Xray 时要求 v2rayN `7.17.1+`、Xray-core `26.2.6+`。分享链接同时包含 Hysteria2 官方的 `pinSHA256` 和 Xray 分享规范的 `pcs`；v2rayN 会把 `pcs` 映射为 `pinnedPeerCertSha256`，链接不再输出已移除的 `allowInsecure`。如果脚本无法读取自签证书指纹，将拒绝生成客户端分享链接，请通过菜单 `2` 重新配置证书。重新生成自签证书后必须重新导入节点。长期使用仍建议优先采用 CA 域名证书模式。
